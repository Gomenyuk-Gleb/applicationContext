...
import com.exeption.NoUniqueBeanException;
...
public class SimpleApplicationContext implements ApplicationContext {
    ...
    @SneakyThrows
    public SimpleApplicationContext(String packageName) {
        ...
        for (Class<?> aClass : typesAnnotatedWith) {
            ...
            String beanName = aClass.getAnnotation(Bean.class).value().equals("")  ? basicName(aClass) : aClass.getAnnotation(Bean.class).value();
            map.put(beanName, o);
        }

        map.entrySet().stream()
                .map(bean -> Map.entry(bean.getValue(), Arrays.asList(bean.getValue().getClass().getDeclaredFields())))
                .filter(fields -> fields.getValue().stream().anyMatch(field -> field.isAnnotationPresent(Inject.class)))
                .forEach(field -> {
    ...
    public <T> Object getBean(String beanName) {
        List<Object> beanEntity = map.entrySet().stream().filter(type -> type.getKey().equals(beanName)).map(Map.Entry::getValue).toList();
    ...
    public <T> Map<String, Object> getAllBeans(Class<T> beanType) {
        ...
        return map.entrySet().stream().filter(val -> checkClass(val.getValue(), beanType)).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }
}
